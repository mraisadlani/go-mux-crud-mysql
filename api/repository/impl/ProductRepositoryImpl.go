...
import (
	"database/sql"
	"errors"
	"github.com/vanilla/go-mux-crud/api/entities"
	"time"
)
...
func (r *ProductRepositoryImpl) Save(prod entities.Product) (bool, error) {
	...
	row, err := r.db.Prepare(`
		INSERT INTO products (name, slug, description, image, price, weight, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`)
	defer row.Close()
...
func (r *ProductRepositoryImpl) Update(uid uint64, prod entities.Product) (bool, error) {
	...
	row, err := r.db.Prepare(`
		UPDATE products SET name=?, slug=?, description=?, image=?, price=?, weight=?, status=?, updated_at=? WHERE id=?
	`)
	defer row.Close()
